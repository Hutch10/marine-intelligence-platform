...
```powershell
cd C:\Users\hetfw\marine\apps\api
...
| Check | Threshold |
|-------|-----------|
| Latest observation timestamp | Must be within **6 hours** of now |
| Latest reef alert timestamp | Must be within **48 hours** of now |
| Consecutive GHA failures | **2** → page operator |
| `ingest:live` exit code | `1` = ingest failure, `2` = persistence failure |
