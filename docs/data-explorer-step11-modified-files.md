...
## apps/web/app/api/data-explorer/presets/audit/route.ts
```ts
...
## apps/web/app/api/data-explorer/presets/audit/route.test.ts
```ts
...
test("GET scopes personal preset activity reads to the authenticated station admin actor", async () => {
  ...
  const response = await GET(
    new Request("http://localhost/api/data-explorer/presets/audit?scope=personal&presetId=preset-1&actorId=operator-11&limit=10"),
  );
