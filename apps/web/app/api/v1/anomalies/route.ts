import { NextRequest } from "next/server";
import { logApiUsageSafely, requireApiKeyAuth } from "../_auth";
import { jsonPublicApiError, jsonPublicApiResponse } from "../_responses";

function requireApiBase(): string {
  const configured = process.env.MARINE_API_BASE_URL?.trim().replace(/\/$/, "");
  if (!configured) {
    if (process.env.NODE_ENV === "production" || process.env.VERCEL) {
      throw new Error("MARINE_API_BASE_URL is not configured");
    }
    return "http://localhost:4000";
  }
  return configured;
}

export async function GET(request: NextRequest) {
  const startedAt = Date.now();
  const authResult = await requireApiKeyAuth(request);

  if (!authResult.ok) {
    return authResult.response;
  }

  const params = new URLSearchParams();
  const stationId = request.nextUrl.searchParams.get("stationId");
  const since = request.nextUrl.searchParams.get("since");
  const limit = request.nextUrl.searchParams.get("limit");
  if (stationId) params.set("stationId", stationId);
  if (since) params.set("since", since);
  if (limit) params.set("limit", limit);

  const qs = params.size > 0 ? `?${params.toString()}` : "";
  let upstreamStatus = 502;
  let upstreamJson: unknown;

  try {
    const upstream = await fetch(`${requireApiBase()}/anomalies${qs}`, {
      headers: { Accept: "application/json" },
    });
    upstreamStatus = upstream.status;
    upstreamJson = await upstream.json();
  } catch {
    await logApiUsageSafely({
      keyId: authResult.key.id,
      route: "/api/v1/anomalies",
      statusCode: 502,
      durationMs: Date.now() - startedAt,
      requestAt: startedAt,
    });
    return jsonPublicApiError(502, "anomalies_unavailable", "Anomalies service unreachable", {
      retryable: true,
      rateLimit: authResult.rateLimit,
    });
  }

  await logApiUsageSafely({
    keyId: authResult.key.id,
    route: "/api/v1/anomalies",
    statusCode: upstreamStatus,
    durationMs: Date.now() - startedAt,
    requestAt: startedAt,
  });

  if (upstreamStatus !== 200) {
    const message =
      typeof upstreamJson === "object" &&
      upstreamJson !== null &&
      "message" in upstreamJson &&
      typeof (upstreamJson as { message: unknown }).message === "string"
        ? (upstreamJson as { message: string }).message
        : "Anomalies request failed";
    return jsonPublicApiError(
      upstreamStatus,
      upstreamStatus >= 500 ? "anomalies_unavailable" : "anomalies_invalid_request",
      message,
      { retryable: upstreamStatus >= 500, rateLimit: authResult.rateLimit },
    );
  }

  return jsonPublicApiResponse(upstreamJson, {
    status: upstreamStatus,
    rateLimit: authResult.rateLimit,
  });
}
