...
test('investigations route returns honest degraded-state on DB failure', () => {
  const result = buildInvestigationsRouteResponse({ source: 'mock', fallbackReason: 'db_query_failed' });
  assert.equal(result.status, 200);
  assert(result.json.workspace.evidenceItems instanceof Array);
  assert(result.telemetry.fallbackReason === 'db_query_failed');
});

test('investigations route rejects absurd pagination and invalid IDs', () => {
  // Simulate invalid input (pagination, IDs)
  // This is a placeholder; replace with actual route handler if available
  assert.doesNotThrow(() => buildInvestigationsRouteResponse({ source: 'mock', fallbackReason: 'db_query_failed' }));
});

test('investigations route does not fabricate confidence or evidence', () => {
  const result = buildInvestigationsRouteResponse({ source: 'mock', fallbackReason: 'db_query_failed' });
